function [CMC_Mean,CMCArr,CMC_Mean_Antith_Var,CMCAntithArr...
          ...
          ,CorrCVBase,CorrCVAnt]  = MonteGTATVolStoch(S0,K,r,vol,n,m,alpha,eta,volhat)
...
%
         ...
         vol0 = vol; % setting so that first vol will be the implied
                ...
                for j =1:m % running multiple simulations (paths)
                     ...
                     Zvol = normrnd(ZeroArr,OnesArr); %Stochastic volatility GRV
                     Stockprices = NanArr;
                     ...
                     timestep = ZeroArr;
                     timestep(1) = 0;
                     St=S0; %setting intial stock price
                     StNeg = S0;
                     
                    
                    for i = 1:n %Stock path until maturity day
                        
                        %Calculating Stochastic volatility
                                              
                        muvol = alpha*(vol-volhat);
                        sigvol = 0.5*(eta^2);
                        
                        
                           vola = vol*exp((muvol-sigvol)*dt);
                        volexpo = eta*sqrt(dt);
                        
                        vol = vola * exp(volexpo*Zvol(i));
                        %this is the stochastic volatility

                        sig = 0.5*(vol^2); %consatnt value in calculation,
                        
                        
                            if i == 1 %this is so starts at intial vol
...
%St is the discrete form approximation
                        ...
                        %efficency and for clarity
                        Sta = St*exp((r-sig)*dt);
                        StaNeg = StNeg*exp((r-sig)*dt);
                        Stbexpo = vol*sqrt(dt);
                        
                        St = Sta * exp(Stbexpo*Z(i));
                        StNeg = StaNeg * exp(Stbexpo*ZNeg(i)); %the stock price if random number was negative
                        ...
                        if i == n %when at maturity date
                            ...
                            %Need Neg in order to calculate the covariance
                            CMCNeg = exp(-r*T)*max([StockpricesNeg(end)-K,0.]);%exponential factor is the discount factor
                            %ASIAN OPTION
                            
                            CMCAntith = (CMC+CMCNeg)/2; %Antithetic Price
                            ...
                            GT(j) = exp(sum(log(StockpricesPlus))/length(StockpricesPlus)); %geometric average    Geo average final stock price
                            GT(j) = GT(j) - 1;
                           ...
                           %Arithmetic var reduction asian
                            ...
                            % MAturity Stock price Cen be storred for Floating asian option price
...
%for investigating variance reduction when combining control variate with
%anithetic
